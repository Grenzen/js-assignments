...
function extractNameFromTemplate(value) {
    ...
    let sign = value.search(/,/g);
	return value.slice(sign+1, value.length-1).trim();
}
...
function encodeToRot13(str) {
    ...
    for(let i = 0; i < len; i++) {
        ...
        if (str[i] === ' ' || str[i] === '!' || str[i] === '?') {     
            ...
            root13 = str.charCodeAt(i) + 13;
   
            if ((root13-13) <= 90 && root13 >= 97 || root13 > 90 && root13 < 97 || root13 > 122) {
                root13 -= 26;
            }
            nStr += String.fromCharCode(root13);
        }
    }
    return nStr;
}
 ...
 *   isString({}) => false
 *   isString('test') => true
 *   isString(new String('test')) => true //? this is an object(!?), expected false
 */

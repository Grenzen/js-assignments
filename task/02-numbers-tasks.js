...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let magn1 = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
    let magn2 = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));

    return Math.acos(cross/ ( magn1 * magn2));
}
 ...
 */
function isPrime(n) {
    let mid = Math.round(Math.sqrt(n));
    for (let i = 2; i <= mid; i++) {
        if (n % i === 0) {
            return false;
        }
